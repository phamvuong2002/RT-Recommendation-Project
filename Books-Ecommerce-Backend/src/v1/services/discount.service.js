...
const { DISCOUNT_WALLET_STATUS, DISCOUNT_TYPE, DISCOUNT_APPLY_TO } = require('../const/discount');
const { BadRequestError, NotFoundError } = require('../core/error.response');
const db = require('../models/sequelize/models');
const { checkDiscountCode, checkAndupdateDiscountWallet } = require('../utils/checkDiscountCode');
/*
...
class DiscountService {
    ...
    static async createDiscount(payload){
        ...
        */
        ...
        if(discountType !== DISCOUNT_TYPE.FIXED_AMOUNT && discountType !== DISCOUNT_TYPE.PERCENTAGE)
            throw new BadRequestError('Discount type is invalid');

        if(discountType === DISCOUNT_TYPE.PERCENTAGE && discountValue > 100.00){
        ...
        if(discountMaxUsesPerUser < 0)
            throw new BadRequestError('Discount max uses per user must be greater than 0');

        if(discountApplyTo !== DISCOUNT_APPLY_TO.SPECIFIC && discountApplyTo !== DISCOUNT_APPLY_TO.ALL)
            throw new BadRequestError('Discount Apply To is invalid');

        if(discountApplyTo === DISCOUNT_APPLY_TO.SPECIFIC && discountProductIds.length === 0)
    ...
    static async applyDiscount({userId, discountId, product={} ,order={}}) {
        ...
        if(!discountWallet) throw new NotFoundError('Discount not found in your wallet');
        ...
        if(discountWallet.dataValues.dw_discount_status === DISCOUNT_WALLET_STATUS.OUTDATE)
            throw new BadRequestError('Discount is out of date');

        if(discountWallet.dataValues.dw_discount_quatity <= discountWallet.dataValues.dw_discount_used)
            throw new BadRequestError('Discount usage limit has been reached');

        const discount = await db.discount.findByPk(discountWallet.dataValues.dw_discount_id);
        //Check discount
        ...
        if(discount.dataValues.discount_apply_to === DISCOUNT_APPLY_TO.SPECIFIC){
            ...
            if(discount.dataValues.discount_min_order_value > (product.price * product.quantity))
            ...
            if(!discount.dataValues.discount_product_ids.includes(product.productId)){
        ...
        await discount.save();
        if(discount.dataValues.discount_count_used >= discount.dataValues.discount_max_uses){
            discount.set({
        ...
        if(discount.dataValues.discount_is_active === 0 ||
            discountWallet.dataValues.dw_discount_quatity <= discountWallet.dataValues.dw_discount_used) {
            await discountWallet.set({
    ...
    static async getDiscountAmount({discountId, discountCode, userId, product= {}, order={}}) {
        ...
        if(discount.dataValues.discount_apply_to === DISCOUNT_APPLY_TO.SPECIFIC){
            ...
            if(!discount.dataValues.discount_product_ids.includes(product.productId)){
                ...
                discount.dataValues.discount_value : product.price * product.quantity * (discount.dataValues.discount_value / 100);
    ...
    static async getDiscountWallet({userId}) {
        const listDiscounts = await db.discount_wallet.findAll({
            ...
            include: [
              {
                model: db.discount,
                attributes: ['discount_name', 'discount_des', 'discount_type','discount_value', 
                'discount_code', 'discount_end_date', 'discount_min_order_value', 'discount_apply_to']
              }
            ],
            attributes: ['dw_discount_id', 'dw_discount_quatity', 'dw_discount_used', 'dw_discount_status']
        });
    ...
    static async collectDiscount({userId, discountId}){
        ...
        if(!discountInWallet){
            ...
            if(discountInWallet.dataValues.dw_discount_status === DISCOUNT_WALLET_STATUS.INACTIVE){
                //check max uses
                if(discountInWallet.dataValues.dw_discount_used < foundDiscount.dataValues.discount_max_uses_per_user){
                    await discountInWallet.set({
                        dw_discount_quatity: discountInWallet.dataValues.dw_discount_quatity + 1,
                ...
                //check max uses
                if(discountInWallet.dataValues.dw_discount_quatity < foundDiscount.dataValues.discount_max_uses_per_user){
                    await discountInWallet.set({
                        dw_discount_quatity: discountInWallet.dataValues.dw_discount_quatity + 1
                    })
                    await discountInWallet.save();
                ...
                if(discountInWallet.dataValues.dw_discount_quatity > discountInWallet.dataValues.dw_discount_used) {
                    return discountInWallet 
                }
