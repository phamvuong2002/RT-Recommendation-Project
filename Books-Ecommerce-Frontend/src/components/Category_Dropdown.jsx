      ...
      <div className=" sm:px-0 max-w-screen-xl h-dvh grid grid-cols-5 lg:hidden rounded-[5px] bg-gray-100 border-2 shadow-sm  overflow-y-scroll">
        ...
        {!isLoading && (
          <Tab.Group vertical>
              ...
              {cate.map((category) => (
                ...
                >
                  <h2 className="items-center flex flex-col text-[9px] sm:text-base">
                    {category.id == '1' ? (
                      <FaBook className="text-red-500 text-xs sm:text-base mb-2" />
                    ) : (
                      <BiWorld className="text-indigo-500 text-xs sm:text-base mb-2" />
                    )}
                    {category.name}
                  </h2>
                </Tab>
            ...
            <Tab.Panels className=" col-span-4 ">
              {cate.map((main_cates, idx) => (
                <Tab.Panel key={idx} className={classNames(' bg-gray-100 p-2')}>
                  {/* Thể loại */}
