            ...
            {/* Lặp qua từng nhóm sản phẩm */}
            {
                Object.entries(groupedProducts).map(([publisherID, products]) => (
                    <div key={publisherID} className="flex flex-col gap-1">
                        {/* Nhà xuất bản */}
                        <div className="w-full h-12 max-h-12 flex items-center p-2 bg-red-50 font-inter gap-8">
                            ...
                            {/* Remove button */}
                            <button className="" onClick={() => handleDeletePublisherProducts(publisherID)}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 text-gray-500 xl:hover:text-red-500">
