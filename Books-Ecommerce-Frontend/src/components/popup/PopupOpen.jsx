  ...
  const handleError = () => {
    return generateContent(
      ...
      ErrorHandling?.message ||
        'Có một vài sự cố khi xử lý. Vui lòng thử lại sau!',
    );
  };
        ...
        >
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center outline-none focus:outline-none">
            ...
            >
              <div
                tabIndex={0}
                className="bg-gray-50 inline-block align-middle px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all max-w-lg w-full"
              >
                {customContent}

                <div
                  className={`mt-5 flex justify-between ${Option ? '' : 'hidden'}`}
                >
                  <button
                    type="button"
                    ...
                    disabled={loading}
                  >
                    {loading ? 'Loading...' : Option?.yes}{' '}
                    {/* Hiển thị "Loading" khi đang fetch, ngược lại hiển thị label của nút */}
                  </button>
                  <button
                    type="button"
