  ...
  const handleUpdateEmail = async () => {
    ...
    if (isValidEmail.status) {
      ...
      if (isRegistered.status === 200) {
        ...
        setMessage(
          'Đã xảy ra lỗi trong quá trình cập nhật. Vui lòng thử lại sau',
        );
      }
    } else {
  ...
  const handleUpdatePhone = async () => {
    ...
    if (isValidPhoneNumber(value)) {
      ...
      if (isRegistered.status === 200) {
        if (isRegistered.metadata.isUsed) {
          setMessage(
            'Số điện thoại đã được đăng ký, vui lòng cập nhật bằng Số điện thoại khác',
          );
          return;
        } else {
          setPhone(value);
        ...
        content={
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-center font-semibold">
              Nhập {isAddEmail ? 'Email' : 'Số Điện Thoại'}
            </div>
            <div className="">
              <label className="text-gray-400 text-xs" htmlFor="Nhập email mới">
                Nhập {isAddEmail ? 'Email' : 'Số Điện Thoại'} mới
              </label>
            ...
            <div className="flex items-center mt-4 justify-center">
              <button
                className={`${isAddEmail ? '' : 'hidden'} w-4/5 h-10 rounded-full text-white font-normal xl:font-semibold xl: text-base bg-gradient-to-r from-pink-500 to-red-500 transition-all xl:hover:from-red-400 xl:hover:to-pink-400`}
